defmodule AeMdw.Db.Stream do
  ...
  def t() do
    ...
    true = Enum.map(txis, &{:type, {:spend_tx, &1}, nil}) == (:forward |> DBS.map(~t[type]) |> Enum.take(10))
    true = Enum.map(first_10_tx_recs, &{:time, {Model.tx(&1, :time), Model.tx(&1, :index)}, nil}) == (:forward |> DBS.map(~t[time]) |> Enum.take(10))
    genesis_pk = <<144, 125, 123, 13, 183, 6, 234, 74, 192, 116, 177, 35, 130, 58, 45, 133, 185,
  14, 29, 143, 113, 100, 77, 100, 127, 133, 98, 225, 46, 110, 14, 75>>
    ...
    # GEN tests
    [{1, -1}, {1, 0}] = {:gen, 1} |> DBS.map(~t[block], &Model.block(&1, :index)) |> Enum.to_list
    ...
    # TIME tests
    scope = {:time, 1545163941077..1545168969077}
    ...
    # TYPE tests
    txis = Enum.to_list(3000000..3000009)
    ^txis = {:txi, 3_000_000..4_000_000} |> DBS.map(~t[type], &elem(Model.type(&1, :index), 1)) |> Enum.take(10)
    ^txis = {:time, 1567585929155..1567585949232} |> DBS.map(~t[type], &elem(Model.type(&1, :index), 1)) |> Enum.take(20)
    all_txis = {:gen, 134252..134253} |> DBS.map(~t[type], &elem(Model.type(&1, :index), 1)) |> Enum.to_list
    true = MapSet.subset?(MapSet.new(txis), MapSet.new(all_txis))


    # TXI tests
    [123_456] = {:txi, 123_456} |> DBS.map(~t[tx], :txi) |> Enum.take(10)
    true = Enum.to_list(10_000..10_009) == ({:txi, 10_000..10_009} |> DBS.map(~t[tx], :txi) |> Enum.take(100))
    true = Enum.to_list(10_009..10_000) == ({:txi, 10_009..10_000} |> DBS.map(~t[tx], :txi) |> Enum.take(100))
