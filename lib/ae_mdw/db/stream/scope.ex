defmodule AeMdw.Db.Stream.Scope do
  ...
  @time_scope_max_diff_msecs 1000 # second
  ...
  def merge_range({l, r}) do
    keys = fn {:range, {l, r}} -> [l, r]; nil -> []; x -> [x] end
  ...
  def translate1({:time, i}, :time) when is_integer(i) and i < 0,
    do: raise ArgumentError, message: "time (in milliseconds) is non-null integer, got #{inspect i}"
  ...
  def on_block_txis(f) do
    fn b1, b2 ->
      txi1 = Model.block(b1, :tx_index) # |> prx("////////// [on_block_txis] TXI1")
  ...
  def scan_microblock(h, limit_i, :left),
    do: collect_keys(~t[block], nil, {h, -1}, &next/2,
          fn {k, mbi}, nil when k <= limit_i ->
              mbi >= 0 && {:halt, {k, mbi}} || {:cont, nil}
             {_, _}, nil ->
              {:halt, nil}
          end)
  def scan_microblock(h, limit_i, :right),
    do: collect_keys(~t[block], nil, {h + 1, -1}, &prev/2,
          fn {k, mbi}, nil when k >= limit_i ->
              mbi >= 0 && {:halt, {k, mbi}} || {:cont, nil}
             {_, _}, nil ->
              {:halt, nil}
