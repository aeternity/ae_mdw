defmodule AeMdw.Db.NameInvalidationMutation do
  ...
  @type table_records :: {Model.table(), [Model.record()]}
  ...
  def execute(%__MODULE__{keys_delete: keys_delete, records_write: records_write}, txn) do
    Enum.each(keys_delete, fn {tab, keys} ->
      Enum.each(keys, fn key -> Name.cache_through_delete(txn, tab, key) end)
    end)

    Enum.each(records_write, fn {tab, records} ->
      Enum.each(records, fn record -> Name.cache_through_write(txn, tab, record) end)
    end)
  end
end
