defmodule AeMdwWeb.GraphQL.Resolvers.TransactionResolver do
  ...
  defp fetch_by_index(state, idx_str) do
    case Integer.parse(idx_str) do
      ...
      _ -> {:error, "invalid_transaction_id"}
    end
  end
  ...
  with {:ok, query} <- build_query_filters(account, type),
     {:ok, {prev, txs, next}} <- Txs.fetch_txs(state, pagination, range, query, cursor, render_v3?: true) do
  ...
  # ---------------- Transactions Count ----------------
  @spec transactions_count(any, map(), Absinthe.Resolution.t()) :: {:ok, integer()} | {:error, String.t()}
  ...
  @spec pending_transactions(any, map(), Absinthe.Resolution.t()) :: {:ok, map()} | {:error, String.t()}
  ...
  @spec pending_transactions_count(any, map(), Absinthe.Resolution.t()) :: {:ok, integer()} | {:error, String.t()}
  ...
  @spec micro_block_transactions(any, map(), Absinthe.Resolution.t()) :: {:ok, map()} | {:error, String.t()}
  ...
  with {:ok, query} <- build_query_filters(account, type),
     {:ok, {prev, txs, next}} <- Txs.fetch_micro_block_txs(state, hash, query, pagination, cursor, render_v3?: true) do
      data = Enum.map(txs, &atomize_tx/1)
      {:ok, %{prev_cursor: cursor_val(prev), next_cursor: cursor_val(next), data: data}}
    else
  ...
  @spec key_block_micro_blocks(any, map(), Absinthe.Resolution.t()) :: {:ok, map()} | {:error, String.t()}
  ...
  defp normalize_micro_block(block) do
    %{
      ...
      micro_block_index: Map.get(block, :micro_block_index) || Map.get(block, "micro_block_index"),
      transactions_count: Map.get(block, :transactions_count) || Map.get(block, "transactions_count"),
  ...
  defp add_type(query, type) do
    case Validate.tx_type(type) do
      {:ok, tx_type} -> {:ok, Map.update(query, :types, MapSet.new([tx_type]), &MapSet.put(&1, tx_type))}
      {:error, _} -> {:error, ErrInput.Query.exception(value: "invalid_type")}
    end
  end

  defp atomize_tx(tx_map) do
    tx_map = Enum.reduce(tx_map, %{}, fn {k, v}, acc when is_binary(k) -> Map.put(acc, String.to_atom(k), v) ; {k,v}, acc -> Map.put(acc,k,v) end)
    encoded_tx =
    ...
    inner = case Jason.decode(encoded_tx) do {:ok, m} -> m; _ -> %{} end
    # promote common fields if present
    promoted = Enum.reduce(~w(fee type gas gas_price nonce sender_id recipient_id amount ttl payload)a, %{}, fn key, acc ->
      kstr = to_string(key)
