defmodule AeMdwWeb.GraphQL.Resolvers.Aex141Resolver do
  @moduledoc """
  AEX141 resolvers: contracts, owners/tokens/templates, and transfers.
  """
  alias AeMdw.{Aex141, AexnTokens, AexnTransfers}
  ...
  # -------------- Count --------------
  @spec aex141_count(any, map(), Absinthe.Resolution.t()) :: {:ok, integer()} | {:error, String.t()}
  def aex141_count(_p, _args, %{context: %{state: %State{} = state}}) do
    case State.get(state, Model.Stat, Stats.aexn_count_key(:aex141)) do
      {:ok, {:stat, _key, count}} -> {:ok, count}
      :not_found -> {:ok, 0}
    end
  end
  def aex141_count(_, _args, _), do: {:error, "partial_state_unavailable"}

  # -------------- Contracts list/single --------------
  @spec aex141_contracts(any, map(), Absinthe.Resolution.t()) :: {:ok, map()} | {:error, String.t()}
  def aex141_contracts(_p, args, %{context: %{state: %State{} = state}}) do
    ...
    case AexnTokens.fetch_contracts(state, pagination, :aex141, query, order_by, cursor, true) do
      ...
      {:error, _} -> {:error, "aex141_contracts_error"}
    end
  end
  def aex141_contracts(_, _args, _), do: {:error, "partial_state_unavailable"}

  @spec aex141_contract(any, map(), Absinthe.Resolution.t()) :: {:ok, map()} | {:error, String.t()}
  def aex141_contract(_p, %{id: id}, %{context: %{state: %State{} = state}}) do
    case AexnTokens.fetch_contract(state, :aex141, id, true) do
      {:ok, contract} -> {:ok, contract}
      {:error, %ErrInput.NotFound{}} -> {:error, "contract_not_found"}
      {:error, _} -> {:error, "aex141_contract_error"}
    end
  end
  def aex141_contract(_, _args, _), do: {:error, "partial_state_unavailable"}

  # -------------- Transfers --------------
  @spec aex141_transfers(any, map(), Absinthe.Resolution.t()) :: {:ok, map()} | {:error, String.t()}
  def aex141_transfers(_p, args, %{context: %{state: %State{} = state}}) do
    ...
    query = %{}
    query = maybe_put(query, "from", sender)
    query = maybe_put(query, "to", recipient)
    ...
    case AexnTransfers.fetch_aex141_transfers(state, pagination, cursor, query) do
      ...
      {:error, %ErrInput.Cursor{}} -> {:error, "invalid_cursor"}
      {:error, %ErrInput.Query{}} -> {:error, "invalid_filter"}
      {:error, _} -> {:error, "aex141_transfers_error"}
    end
  end
  def aex141_transfers(_, _args, _), do: {:error, "partial_state_unavailable"}

  @spec aex141_contract_transfers(any, map(), Absinthe.Resolution.t()) :: {:ok, map()} | {:error, String.t()}
  def aex141_contract_transfers(_p, %{id: id} = args, %{context: %{state: %State{} = state}}) do
    ...
    with {:ok, {_k, _a}} <- validate_filter(filter),
      ...
      case AexnTransfers.fetch_contract_transfers(state, contract_pk, normalize_filter(filter), pagination, cursor) do
        {:ok, {prev, keys, next}} ->
          data = Enum.map(keys, &AeMdwWeb.AexnView.contract_transfer_to_map(state, :aex141, elem(filter, 0), &1, true))
  ...
  @spec aex141_contract_tokens(any, map(), Absinthe.Resolution.t()) :: {:ok, map()} | {:error, String.t()}
  def aex141_contract_tokens(_p, %{id: id} = args, %{context: %{state: %State{} = state}}) do
    ...
    with {:ok, contract_pk} <- AeMdw.Validate.id(id, [:contract_pubkey]),
         {:ok, {prev, items, next}} <- Aex141.fetch_collection_owners(state, contract_pk, cursor, pagination) do
      {:ok, %{prev_cursor: cursor_val(prev), next_cursor: cursor_val(next), data: items}}
    else
      {:error, %ErrInput.Cursor{}} -> {:error, "invalid_cursor"}
      {:error, %ErrInput.NotFound{}} -> {:error, "contract_not_found"}
  ...
  @spec aex141_contract_templates(any, map(), Absinthe.Resolution.t()) :: {:ok, map()} | {:error, String.t()}
  def aex141_contract_templates(_p, %{id: id} = args, %{context: %{state: %State{} = state}}) do
    ...
    with {:ok, contract_pk} <- AeMdw.Validate.id(id, [:contract_pubkey]),
         {:ok, {prev, items, next}} <- Aex141.fetch_templates(state, contract_pk, cursor, pagination) do
      {:ok, %{prev_cursor: cursor_val(prev), next_cursor: cursor_val(next), data: items}}
    else
      {:error, %ErrInput.Cursor{}} -> {:error, "invalid_cursor"}
  ...
  @spec aex141_template_tokens(any, map(), Absinthe.Resolution.t()) :: {:ok, map()} | {:error, String.t()}
  def aex141_template_tokens(_p, %{id: id, template_id: template_id} = args, %{context: %{state: %State{} = state}}) do
    limit = clamp_limit(Map.get(args, :limit, 50))
    cursor = Map.get(args, :cursor)
    pagination = {:backward, false, limit, not is_nil(cursor)}
    ...
    with {:ok, contract_pk} <- AeMdw.Validate.id(id, [:contract_pubkey]),
         ...
         {:ok, {prev, items, next}} <- Aex141.fetch_template_tokens(state, contract_pk, template_id_int, cursor, pagination) do
      {:ok, %{prev_cursor: cursor_val(prev), next_cursor: cursor_val(next), data: items}}
    else
      {:error, %ErrInput.Cursor{}} -> {:error, "invalid_cursor"}
  ...
  # -------------- Token detail --------------
  @spec aex141_token_detail(any, map(), Absinthe.Resolution.t()) :: {:ok, map()} | {:error, String.t()}
  def aex141_token_detail(_p, %{contract_id: cid, token_id: tid}, %{context: %{state: %State{} = state}}) do
    case Aex141.fetch_nft(state, cid, to_string(tid), v3?: true) do
      ...
      {:error, _} -> {:error, "aex141_token_detail_error"}
    end
  end
