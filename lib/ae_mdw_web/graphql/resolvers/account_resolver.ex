defmodule AeMdwWeb.GraphQL.Resolvers.AccountResolver do
    ...
    with {:ok, account_pk} <- Validate.id(id) do
      case State.get(state, Model.AccountBalance, account_pk) do
        {:ok, acc_balance} ->
          ...
          names_count =
            case State.get(state, Model.AccountNamesCount, account_pk) do
  ...
  def accounts(_parent, args, %{context: %{state: %State{} = state}}) do
    ...
    data =
      Enum.map(accounts, fn {id_enc, balance, creation_time} ->
    ...
    if length(acc) >= limit do
      ...
      case State.prev(state, Model.ActiveNameOwner, key) do
        {:ok, {^account_pk, name_hash}} ->
          collect_account_names(state, account_pk, {account_pk, name_hash}, limit, [
            name_hash | acc
          ])
  ...
  defp name_entry(state, {_account_pk, name_hash}) do
    ...
    expire_h =
      case State.get(state, Model.ActiveNameExpiration, name_hash) do
  ...
  @spec account_aex9_balances(any, map(), Absinthe.Resolution.t()) ::
  ...
  @spec account_aex141_tokens(any, map(), Absinthe.Resolution.t()) ::
          {:ok, map()} | {:error, String.t()}
  def account_aex141_tokens(_p, %{id: id} = args, %{context: %{state: %State{} = state}}) do
    ...
    case AeMdw.Aex141.fetch_owned_tokens(state, id, cursor, pagination, query) do
      ...
      {:error, %AeMdw.Error.Input.Cursor{}} ->
        {:error, "invalid_cursor"}

      {:error, %AeMdw.Error.Input.Query{}} ->
        {:error, "invalid_filter"}

      {:error, _} ->
        {:error, "aex141_tokens_error"}
    end
  end
  ...
  defp collect_backward(state, cursor_pk, limit) do
    start_pk =
      case cursor_pk do
        ...
        pk ->
          case State.prev(state, Model.AccountBalance, pk) do
            {:ok, prev_pk} -> prev_pk
            :none -> nil
          end
      end
  ...
  defp do_collect(state, pk, limit, acc) do
    ...
    next_key =
      case State.prev(state, Model.AccountBalance, account_pk) do
        {:ok, prev_pk} -> prev_pk
        :none -> nil
      end
