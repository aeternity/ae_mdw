defmodule AeMdwWeb.Query.Optimizer do
  ...
  def add_roots_checks({tx_type, {:or, pos_variants}}, root_groups) do
    Enum.reduce(pos_variants, root_groups, fn {_, variant}, root_groups ->
      case sort_variant(tx_type, variant) do
        ...
        [{pubkey, pos} | checks] ->
          ...
          Map.update(root_groups, root, {:or, [{:and, checks}]}, fn {:or, all_root_checks} ->
            {:or,
  ...
  def sort_variant(tx_type, [_ | _] = variant) do
    sorted =
      Enum.reduce_while(variant, :gb_sets.new(), fn bound, acc ->
