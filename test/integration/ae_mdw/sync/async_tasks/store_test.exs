defmodule Integration.AeMdw.Sync.AsyncTasks.StoreTest do
  ...
  describe "save_new/2 and fetch_unprocessed/1 success" do
    test "for an unprocessed task" do
      ...
      assert 1 == Enum.count(tasks, fn Model.async_tasks(args: args) -> args == [@contract_pk] end)
    end
    ...
    test "for a task being processed" do
      ...
      assert Model.async_tasks(index: task_index) = Enum.find(tasks_before, fn Model.async_tasks(args: args) -> args == [@contract_pk] end)
      Store.set_processing(task_index)
      tasks_after = Store.fetch_unprocessed(1000)
      assert nil == Enum.find(tasks_after, fn Model.async_tasks(args: args) -> args == [@contract_pk] end)
    end
  end
