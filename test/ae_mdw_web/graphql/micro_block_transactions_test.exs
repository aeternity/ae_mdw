defmodule AeMdwWeb.GraphQL.MicroBlockTransactionsTest do
        ...
        h when is_integer(h) and h >= 0 ->
          case State.get(st, Model.Block, {h, 0}) do
            {:ok, Model.block(hash: mb_hash)} ->
              :aeser_api_encoder.encode(:micro_block_hash, mb_hash)
  ...
  test "micro_block_transactions page + cursor" do
    ...
    if st do
      ...
      if mb_hash_enc do
        {:ok, res} =
          Absinthe.run(
            "{ microBlockTxs: micro_block_transactions(hash: \"#{mb_hash_enc}\", limit: 3){ data { hash txIndex: tx_index } nextCursor prevCursor } }",
            @schema,
            context: %{state: st}
          )
        ...
        if nc = page["nextCursor"] do
          {:ok, res2} =
            Absinthe.run(
              "{ micro_block_transactions(hash: \"#{mb_hash_enc}\", limit:3, cursor: \"#{nc}\"){ data { txIndex: tx_index } } }",
              @schema,
              context: %{state: st}
            )
  ...
  test "micro_block_transactions invalid hash" do
    ...
    {:ok, res} =
      Absinthe.run("{ micro_block_transactions(hash: \"mh_invalid\"){ data { hash } } }", @schema,
        context: %{state: st}
      )
        ...
        assert(
          Enum.any?(
            ...
            &(&1.message in [
                ...
                "micro_block_transactions_error"
              ])
          )
        )
  end
