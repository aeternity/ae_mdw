defmodule AeMdwWeb.GraphQL.NumericTypesTest do
  ...
  test "numeric leaf types are integers where expected" do
    ...
    if st do
      ...
      {:ok, status_res} =
        run(
          "{ status { last_synced_height last_key_block_time total_transactions pending_transactions } }",
          st
        )
      ...
      {:ok, kb_res} =
        run(
          "{ keyBlocks: key_blocks(limit:1){ data { height time micro_blocks_count transactions_count beneficiary_reward } } }",
          st
        )
      ...
      {:ok, tx_res} =
        run(
          "{ txs: transactions(limit:1){ data { hash block_height micro_index micro_time tx_index } } }",
          st
        )
      ...
      if mb_hash do
        {:ok, mb_res} =
          run(
            "{ micro_block(hash: \"#{mb_hash}\"){ height time micro_block_index transactions_count gas } }",
            st
          )
