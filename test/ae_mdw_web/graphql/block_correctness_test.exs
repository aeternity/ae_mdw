defmodule AeMdwWeb.GraphQL.BlockCorrectnessTest do
  ...
  describe "key_block field correctness" do
    test "single key_block fields match Blocks context",
         %{state: state, last_gen: last_gen} = ctx do
      ...
      if last_gen == 0 do
        ...
        assert kb["microBlocks"] ==
                 (base_block[:micro_blocks_count] || Map.get(base_block, "micro_blocks_count"))

        assert kb["transactions_count"] ==
                 (base_block[:transactions_count] || Map.get(base_block, "transactions_count"))

        # beneficiary_reward can be nil briefly if sync mid-update; only assert when present both sides
        if base_block[:beneficiary_reward],
          do: assert(kb["beneficiary_reward"] == base_block[:beneficiary_reward])
      end
    end
    ...
    test "limit boundary behaviors (0 => default 20, negative => default 20, >100 => clamp)",
         ctx do
      page0 =
        assert_ok(
          gql(
            "{ a: key0: key_blocks(limit:0){ data { height } } b: keyNeg: key_blocks(limit:-5){ data { height } } c: keyBig: key_blocks(limit:101){ data { height } } }",
            ctx
          ),
          []
        )
    ...
    test "fromHeight only restricts to single height", %{last_gen: last_gen} = ctx do
      if last_gen > 5 do
        ...
        page =
          assert_ok(
            gql("{ key_blocks(fromHeight: #{target}, limit: 40){ data { height } } }", ctx),
            ["key_blocks"]
          )
    ...
    test "toHeight only returns heights <= bound", %{last_gen: last_gen} = ctx do
      ...
      page =
        assert_ok(gql("{ key_blocks(toHeight: #{bound}, limit: 10){ data { height } } }", ctx), [
          "key_blocks"
        ])
    ...
    test "range filter returns only heights within bounds", %{last_gen: last_gen} = ctx do
      if last_gen < 10 do
        ...
        q =
          "{ key_blocks(fromHeight: #{from_h}, toHeight: #{to_h}, limit: 20) { data { height } } }"
  ...
  describe "micro_block correctness" do
    test "micro_block fields match Blocks context for first available",
         %{state: state, last_gen: last_gen} = ctx do
      if last_gen == 0 do
        assert true
      else
          ...
          case State.get(state, Model.Block, {target_h, 0}) do
            {:ok, rec} ->
              ...
              case Blocks.fetch_micro_block(state, enc_hash) do
                {:ok, base_mb} ->
                  q =
                    "{ micro_block(hash: \"#{enc_hash}\") { hash height time microBlockIndex: micro_block_index transactions_count gas } }"
                  ...
                  assert mb["microBlockIndex"] ==
                           (base_mb[:micro_block_index] || base_mb["micro_block_index"])

                  assert mb["transactions_count"] ==
                           (base_mb[:transactions_count] || base_mb["transactions_count"])
    ...
    test "contract meta matches DB (first aexn if any)", %{state: state} = ctx do
      case State.prev(state, Model.AexnContract, nil) do
        {:ok, {aexn_type, contract_pk}} ->
          Model.aexn_contract(meta_info: meta) =
