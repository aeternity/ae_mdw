defmodule AeMdwWeb.GraphQL.PaginationEdgeCasesTest do
  ...
  test "accounts large limit clamps" do
    ...
    if st do
      {:ok, res} =
        Absinthe.run("{ accounts(limit:1000){ data { id } } }", @schema, context: %{state: st})
  ...
  test "names large limit clamps" do
    ...
    if st do
      {:ok, res} =
        Absinthe.run("{ names(limit:500){ data { name } } }", @schema, context: %{state: st})

      data = get_in(res, [:data, "names", "data"]) || []
      # internal clamp 100 in resolver
      assert length(data) <= 100
    else
      assert true
    end
  ...
  test "names invalid cursor tolerates" do
    ...
    if st do
      {:ok, res} =
        Absinthe.run("{ names(cursor: \"!!invalid!!\", limit:1){ data { name } } }", @schema,
          context: %{state: st}
        )
  ...
  test "name not found error" do
    ...
    if st do
      {:ok, res} =
        Absinthe.run("{ name(id: \"nonexistentname.chain\"){ name } }", @schema,
          context: %{state: st}
        )
  ...
  test "invalid account id error" do
    ...
    if st do
      {:ok, res} =
        Absinthe.run("{ account(id: \"invalid_pubkey\"){ id } }", @schema, context: %{state: st})
