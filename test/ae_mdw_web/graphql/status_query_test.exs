defmodule AeMdwWeb.GraphQL.StatusQueryTest do
  ...
  test "status returns richer fields" do
    {:ok, res} =
      run(
        "{ status { last_synced_height partial last_key_block_hash last_key_block_time total_transactions pending_transactions } }"
      )
    ...
    if stat["total_transactions"],
      do: assert(is_integer(stat["total_transactions"]) and stat["total_transactions"] >= 0)

    if stat["pending_transactions"],
      do: assert(is_integer(stat["pending_transactions"]) and stat["pending_transactions"] >= 0)
  end
end
