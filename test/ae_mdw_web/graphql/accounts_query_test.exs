defmodule AeMdwWeb.GraphQL.AccountsQueryTest do
  ...
  defp any_account(state) do
    case State.prev(state, Model.AccountBalance, nil) do
      {:ok, pubkey} ->
        Model.account_balance(balance: balance) =
          State.fetch!(state, Model.AccountBalance, pubkey)
  ...
  test "account query parity" do
    ...
    if st do
      ...
      if acc do
        {:ok, res} =
          Absinthe.run(
            "{ account(id: \"#{acc}\"){ id balance creationTime: creation_time } }",
            @schema,
            context: %{state: st}
          )
  ...
  test "account not found" do
    ...
    if st do
      {:ok, res} =
        Absinthe.run(
          "{ account(id: \"ak_1111111111111111111111111111111111111111111111111\"){ id } }",
          @schema,
          context: %{state: st}
        )
      ...
      assert Enum.any?(
               res.errors || [],
               &(&1.message in ["account_not_found", "invalid_account"])
             )
    else
      assert true
    end
  ...
  test "accounts pagination + parity for first page" do
    ...
    if st do
      {:ok, res} =
        Absinthe.run(
          "{ accounts(limit:5){ data { id balance creationTime: creation_time } nextCursor prevCursor } }",
          @schema,
          context: %{state: st}
        )
      ...
      if page["nextCursor"] do
        {:ok, res2} =
          Absinthe.run(
            "{ accounts(limit:5, cursor: \"#{page["nextCursor"]}\"){ data { id } } }",
            @schema,
            context: %{state: st}
          )
