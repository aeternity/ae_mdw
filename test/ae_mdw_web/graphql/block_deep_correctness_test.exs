defmodule AeMdwWeb.GraphQL.BlockDeepCorrectnessTest do
  ...
  test "first page heights match descending sequence", %{last_gen: last_gen} = ctx do
    ...
    {:ok, %{data: %{"key_blocks" => %{"data" => rows}}}} =
      gql!("{ key_blocks(limit: #{assume_limit}) { data { height } } }", ctx)
  ...
  test "multi-page accumulation yields contiguous heights with no gaps or overlaps",
       %{last_gen: last_gen} = ctx do
    if last_gen < 15 do
      assert true
    else
      limit = 5
  ...
  test "end-of-list has nil nextCursor", %{last_gen: last_gen} = ctx do
    ...
    {:ok, %{data: %{"key_blocks" => %{"nextCursor" => nc}}}} =
      gql!("{ key_blocks(toHeight: #{to_h}, limit: 50) { nextCursor data { height } } }", ctx)
  ...
  defp gen_tx_count(state, gen) do
    case State.get(state, Model.Block, {gen, -1}) do
      {:ok, Model.block(tx_index: first)} ->
        last =
          case State.get(state, Model.Block, {gen + 1, -1}) do
            {:ok, Model.block(tx_index: nxt)} ->
              nxt - 1
  ...
  test "key_block micro_blocks_count & transactions_count recompute matches GraphQL",
    ...
    if last_gen == 0 do
      ...
      {:ok, %{data: %{"key_block" => kb}}} =
        gql!(
          "{ key_block(id: \"#{target}\") { height micro_blocks_count transactions_count } }",
          ctx
        )
  ...
  test "beneficiary_reward non-negative and equals internal render",
       %{state: state, last_gen: last_gen} = ctx do
    if last_gen == 0 do
      assert true
    else
      ...
      {:ok, %{data: %{"key_block" => kb}}} =
        gql!("{ key_block(id: \"#{target}\") { beneficiary_reward } }", ctx)
