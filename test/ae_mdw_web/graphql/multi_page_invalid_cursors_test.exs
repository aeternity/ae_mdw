defmodule AeMdwWeb.GraphQL.MultiPageInvalidCursorsTest do
  ...
  test "accounts multi-page pagination" do
    ...
    if st do
      {:ok, first} =
        Absinthe.run("{ accounts(limit:2){ data { id } nextCursor } }", @schema,
          context: %{state: st}
        )
      ...
      if next do
        {:ok, second} =
          Absinthe.run("{ accounts(limit:2, cursor: \"#{next}\"){ data { id } } }", @schema,
            context: %{state: st}
          )
  ...
  test "names multi-page pagination" do
    ...
    if st do
      {:ok, first} =
        Absinthe.run("{ names(limit:2){ data { name } nextCursor } }", @schema,
          context: %{state: st}
        )
      ...
      if next do
        {:ok, second} =
          Absinthe.run("{ names(limit:2, cursor: \"#{next}\"){ data { name } } }", @schema,
            context: %{state: st}
          )
  ...
  test "auctions invalid cursor error or tolerance" do
    ...
    if st do
      {:ok, first} =
        Absinthe.run("{ auctions(limit:1){ data { name } } }", @schema, context: %{state: st})

      has_data = (get_in(first, [:data, "auctions", "data"]) || []) != []

      if has_data do
        {:ok, bad} =
          Absinthe.run("{ auctions(cursor: \"invalid\", limit:1){ data { name } } }", @schema,
            context: %{state: st}
          )
    ...
    if st do
      case first_name(st) do
        ...
        name ->
          {:ok, bad} =
            Absinthe.run(
              "{ nameHistory(id: \"#{name}\", cursor: \"badcursor\"){ data { height } } }",
              @schema,
              context: %{state: st}
            )
    ...
    if st do
      case first_name(st) do
        ...
        name ->
          {:ok, res} =
            Absinthe.run(
              "{ nameClaims(id: \"#{name}\", cursor: \"not_base64@@\"){ data { sourceTxType } } }",
              @schema,
              context: %{state: st}
            )
