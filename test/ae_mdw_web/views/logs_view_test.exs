defmodule AeMdwWeb.LogsViewTest do
  ...
  describe "render_log/3" do
    ...
    test "formats logs from custom events", %{
      ...
      arg_contract_pk = :crypto.strong_rand_bytes(32)
      arg_token_id = Enum.random(100..999)
      arg_price = Enum.random(1_000_000..999_999_999)

      event_args = %{
        "Listing" => [arg_contract_pk, <<arg_token_id::256>>, <<arg_price::256>>],
        "Offer" => [<<arg_token_id::256>>, arg_contract_pk, <<arg_price::256>>],
        "Trade" => [<<arg_token_id::256>>, <<arg_price::256>>, arg_contract_pk]
      }

      expected_args = %{
        "Listing" => [
          encode_contract(arg_contract_pk),
          to_string(arg_token_id),
          to_string(arg_price)
        ],
        "Offer" => [
          to_string(arg_token_id),
      ...
      {logs, store} = custom_logs_setup(store, event_args)
  ...
  defp custom_logs_setup(store, event_args) do
    block_index = {@height, @mbi}
    contract_pk = :crypto.strong_rand_bytes(32)
    create_txi = Enum.random(1_000..9_999)
    ...
    event_args
    |> Enum.with_index(create_txi + 1)
    |> Enum.map_reduce(store, fn {{event_name, args}, txi}, store ->
