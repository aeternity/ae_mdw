...
- **`BigInt`** — Used for values that may exceed JavaScript's `Number.MAX_SAFE_INTEGER`
  (e.g. token balances, rewards, name fees). Always serialized as a **JSON string**
  (`"12345678901234567890"`). Parse with `BigInt(value)` in JavaScript or an equivalent
  arbitrary-precision library. Accepts both integer literals and numeric strings as input.
