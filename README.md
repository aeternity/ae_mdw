...
Paginated list of activities related to the `:id` account.

Each activity contains 3 values:
- `height` - The height in which the event ocurred
- `activity_type` - The type of event.
- `activity_payload` - An object whos structure depends on the type of event.

For transaction events the activity type will be `<TxType>Event`, and the payload will contain a single object `"tx"` with the transaction itself as displayed in the `/v2/txs` endpoint.
...
$ curl https://mainnet.aeternity.io/mdw/v2/accounts/ak_2nVdbZTBVTbAet8j2hQhLmfNm1N2WKoAGyL7abTAHF1wGMPjzx/activities
{
  "data": [
    {
      "height": 85694,
      "activity_type": "NameUpdateTxEvent",
      "activity_payload": {
        "tx": {
